      ...
      <form action="@{/password_change}" method="post">
        <table id="main">
            ...
            <tr>
              <th>職種</th>
              <td class="cp_iptxt">
              <input type="text" size="16" maxlength='8' name="category" placeholder="職種入力" required disabled/>
              </td>
            </tr>
            <tr>
              <th>性別</th>
              <td >
              <input type="radio" name="sex" value="0" checked="checked" disabled/>男
              <input type="radio" name="sex" value="1" disabled/>女
              <input type="radio" name="sex" value="2" disabled/>その他
              </td>
            </tr>
            <tr>
              <th>名前</th>
              <td class="cp_iptxt">
              <input type="text" size="16" name="name" placeholder="氏名入力" required disabled/>
              </td>
            </tr>
            <tr>
              <th>生年月日</th>
              <td>
              <select id="birthday_year" name="birthday_year" required disabled></select>年
               <select id="birthday_month" name="birthday_month" required disabled></select>月
               <select id="birthday_date" name="birthday_date" required disabled></select>日
              </td>
            </tr>
            <tr>
              <th>TEL</th>
              <td class="cp_iptxt">
              <input type="tel" size="16" maxlength='12' name="tel" placeholder="電話番号入力(数値のみ)" pattern="[\d]*" required disabled/>
              </td>
            </tr>
            <tr>
              <th>E-Mail</th>
              <td class="cp_iptxt">
              <input type="email" size="16" maxlength='25' name="adress" placeholder="メールアドレス入力" required disabled/>
              </td>
            </tr>
            <tr>
              <th>年齢</th>
              ...
              <select id="age" name="age" required disabled></select>歳
              </td>
            </tr>
            <tr>
              <th>入社日</th>
              <td>
              <select id="joined_day_year" name="joined_day_year" required disabled></select>年
               <select id="joined_day_month" name="joined_day_month" required disabled></select>月
               <select id="joined_day_date" name="joined_day_date" required disabled></select>日
              </td>
            </tr>
            <tr>
              <th>管理者権限</th>
              <td>
              <input type="radio" name="admin_auth" value="0"checked="checked" disabled/>無
              <input type="radio" name="admin_auth" value="1" disabled/>有
              </td>
            </tr>
            <tr>
              <th>パスワード</th>
